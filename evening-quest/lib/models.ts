...
export async function saveQuestHistory(
  data: Omit<QuestHistory, "id" | "completedAt">
): Promise<QuestHistory> {
  const quest: QuestHistory = {
    ...data,
  ...
  await docClient.send(
    new PutCommand({
      TableName: TableName.QUEST_HISTORY,
      Item: quest,
    })
  );
...
export async function getQuestHistoryList(): Promise<QuestHistory[]> {
  const result = await docClient.send(
    new ScanCommand({
      TableName: TableName.QUEST_HISTORY,
    })
  );
  return (result.Items as QuestHistory[]) ?? [];
}
